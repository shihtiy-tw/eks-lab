...
export ALIAS=$(aws ssm get-parameters-by-path --path "/aws/service/eks/optimized-ami/$CLUSTER_VERSION/amazon-linux-2/" --recursive --region "$AWS_REGION"| jq -cr '.Parameters[].Name' | grep -v "recommended" | awk -F '/' '{print $8}' | sed -r 's/.*(v[[:digit:]]+)$/\1/' | sort -r | uniq | head -n 1 )
